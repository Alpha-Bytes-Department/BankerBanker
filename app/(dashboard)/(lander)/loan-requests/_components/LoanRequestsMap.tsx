...
const LoanRequestsMap: React.FC<LoanRequestsMapProps> = ({
    ...
    () =>
      loanRequests.filter(
        (request) =>
          ...
          !(request.location.lat === 0 && request.location.lng === 0),
      ),
    [loanRequests],
  );
  ...
  const center = useMemo(() => {
    if (validLoanRequests.length === 0) {
      return { lat: 39.8283, lng: -98.5795 }; // Center of USA
    }

    const totalLat = validLoanRequests.reduce(
      (sum, req) => sum + req.location.lat,
      0,
    );
    const totalLng = validLoanRequests.reduce(
      (sum, req) => sum + req.location.lng,
      0,
    );

    return {
      lat: totalLat / validLoanRequests.length,
      lng: totalLng / validLoanRequests.length,
    };
  }, [validLoanRequests]);
  ...
  if (loadError) {
    return (
      <div className="w-full h-[400px] md:h-[500px] rounded-lg border border-gray-200 bg-gray-50 flex items-center justify-center text-sm text-gray-600">
        Unable to load map.
      </div>
    );
  }
  ...
  if (!isLoaded) {
    return (
      <div className="w-full h-[400px] md:h-[500px] rounded-lg border border-gray-200 bg-gray-50 animate-pulse" />
    );
  }
