...
const Navbar = ({ className = "" }: NavbarProps) => {
  ...
  return (
    <>
      ...
      {/* navbar for mobile */}
      <nav
        className={`fixed inset-0 z-40 bg-black/70 bg-opacity-50 transition-opacity md:hidden ${isMenuOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
            ...
            <nav className="grow p-4">
              <div className="flex flex-col space-y-2">
                {Links &&
                  Links.map((link, idx) => (
                    <div
                      className={`flex items-center gap-5 py-2 px-5 rounded-lg ${link.href === "#home" ? "button-primary" : ""}`}
                      key={idx}
                    >
            ...
            <div className="p-4 border-t border-gray-200 flex justify-between items-center">
              {userData ? (
                <>
                  ...
                  <Button text="Log out" className="button-primary w-full " />
                </>
              ) : (
                <>
