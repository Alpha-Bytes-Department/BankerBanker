...
type StatusCardProps = {
  ...
  statusIconOverride?: React.ReactNode;
};
...
const StatusCard = ({
  ...
  return (
    <div
      className={`flex flex-col border border-[#0000001A] rounded-lg w-60 2xl:w-[344px] gap-5 p-5 h-auto`}
    >
      <div className="flex justify-between items-center">
        <p className="text-base xl:text-lg 2xl:text-xl">
          {titleOverride ?? cardType.title}
        </p>
        <span>{iconOverride ?? cardType.icon}</span>
      </div>
      <div ref={ref} className="text-3xl">
        {display}
      </div>
      <div className="flex items-center gap-2">
        <span>{statusIconOverride ?? cardType.statusIcon}</span>
        <span className="text-sm">
          {statusValue} {statusLabelOverride ?? cardType.status}
        </span>
      </div>
    </div>
  );
